...
def bottleneck(input, kernel_num, strides=1, layer_name='bottleneck', padding_mode='same'):
    ...
    channels = tf.keras.backend.int_shape(conv3)[-1]
    x = SELayer(channels=channels)(conv3)
    shortcut_add = shortcut(fx=x, x=input, layer_name=layer_name)
...
def ResNet_SE(input_shape, nclass, activation='sigmoid', net_name='resnet18'):
    """
        :param net_name:
        :param input_shape:
        :param nclass:
...
if __name__ == '__main__':
    model = ResNet_SE(input_shape=(224, 224, 3), nclass=2, net_name='resnet26')
    model.summary()
